  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user.invitesSentCount == 25 && date > user?.invitesSentTime) {
        res
          .status(400)
          .json({ message: "Max invites already sent for the day" });
      } else {
        ...
        let headers = {
          Authorization:
            "key=AAAAIkbj4C4:APA91bFY3e4nCIaodc-18ruDbz6uu_NEz2pFCSnzkcj9-GV2V802y2Q6kDmsQwh46yaD8c1Cq1CNExpzPydbOJtnHB3icgHf5SHzjkeCRetQWR_lAsBhYi3FMu2S60xajIDWJv9igsJ6",
        ...
        if (user.invitesSentCount === 24) {
          const user = await userModel.findOneAndUpdate(
            { _id: req.user._id },
            { invitesSentTime: Date.now(), $inc: { invitesSentCount: 1 } }
          );
        } else {
          const user = await userModel.findOneAndUpdate(
            { _id: req.user._id },
            { invitesSentCount: 1, invitesSentTime: null }
          );
        }

        res
          .status(201)
          .json({ invite: invite, message: "Invite sent Successfully" });
      }
    } catch (err) {
...
module.exports.getAllInvitesSent = async (req, res) => {
  try {
    const invites = await inviteModel
      .find({ sentBy: req.user._id, inviteStatus: "Pending" })
      .populate("sentTo");

    res
      .status(201)
      .json({ invites: invites, message: "Sent Invites Fetched Successfully" });
  } catch (err) {
    let error = err.message;
    res.status(400).json({ error: error });
  }
...
module.exports.getAllInvitesRecieved = async (req, res) => {
  try {
    const invites = await inviteModel
      .find({ sentTo: req.user._id, inviteStatus: "Pending" })
      .populate("sentBy");
    ...
    res
      ...
      .json({
        invites: invites,
        message: "Recieved Invites Fetched Successfully",
      });
  } catch (err) {
    let error = err.message;
    res.status(400).json({ error: error });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        user.invitesAcceptedCount >= 10 &&
        date > user?.invitesAcceptedTime
      ) {
        res
          .status(400)
          .json({ message: "Max invites already accepted for the day" });
      } else {
        const invite = await inviteModel.findOneAndUpdate(
          { _id: inviteId, sentTo: req.user._id },
          { inviteStatus }
        );
        const notification = await notificationModel.create({
          user: invite.sentBy,
        ...
        if (inviteStatus === "Accepted" && user.invitesAcceptedCount == 9) {
          const user = await userModel.findOneAndUpdate(
            { _id: req.user._id },
            {
              invitesAcceptedTime: Date.now(),
              $inc: { invitesAcceptedCount: 1 },
            }
          );
        } else {
          const user = await userModel.findOneAndUpdate(
            { _id: req.user._id },
            { invitesAcceptedCount: 1, invitesAcceptedTime: null }
          );
        }
  ...
  try {
    ...
    if (search !== "") {
      for (let i = 0; i < invites.length; i++) {
        if (
          invites[i].sentTo.name.toLowerCase() ===
            req.user.name.toLowerCase() &&
          invites[i].sentBy.name.toLowerCase().includes(search.toLowerCase())
        ) {
          delete invites[i].sentTo;
          invites[i].user = invites[i].sentBy;
          delete invites[i].sentBy;
        } else if (
          invites[i].sentBy.name.toLowerCase() ===
            req.user.name.toLowerCase() &&
          invites[i].sentTo.name.toLowerCase().includes(search.toLowerCase())
        ) {
          delete invites[i].sentBy;
    ...
    res
      .status(201)
      .json({ matches: invites, message: "Matches Fetched Successfully" });
  } catch (err) {
    let error = err.message;
    res.status(400).json({ error: error });
  }
